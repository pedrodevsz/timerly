...
const projects = [
  {
    ...
    subjects: [
      { name: "Matemática", topics: ["Funções quadráticas", "Análise combinatória", "Geometria espacial", "Probabilidade"] },
...
async function main() {
  await prisma.userSettings.upsert({
    where: { id: "local" },
    update: {},
    create: { id: "local", name: "Pedro Santos", email: "pedro@email.com" },
  });
  ...
  for (const project of projects) {
    await prisma.project.upsert({
      ...
      create: { id: project.id, name: project.name, description: project.description },
    });
  ...
  if (existingSessions === 0) {
    ...
    const topicOrder = ["Funções quadráticas", "Genética mendeliana", "Interpretação textual", "Brasil República"];
    for (let dayOffset = 0; dayOffset < durations.length; dayOffset++) {
      ...
      await prisma.studySession.create({
        data: { topicId: topic.id, startedAt, endedAt, durationSeconds: durations[dayOffset], status: SessionStatus.COMPLETED },
      });
    }
  }
}
