...
function sessionDto(
  session: Awaited<ReturnType<typeof dashboardRepository.sessions>>[number],
): StudySessionDto {
  return {
    id: session.id,
...
export async function getDashboard(
  ...
  const [sessions, settings] = await Promise.all([
    dashboardRepository.sessions(userId),
  ...
  const dayKeys = sessions.map((session) =>
    startOfLocalDay(session.startedAt, timezone),
  );
  const durationByDay = new Map<string, number>();
  const durationBySubject = new Map<
    string,
    { id: string; name: string; durationSeconds: number }
  >();
  for (const session of sessions) {
    const key = startOfLocalDay(session.startedAt, timezone);
    durationByDay.set(
      key,
      (durationByDay.get(key) ?? 0) + session.durationSeconds,
    );
    const subject = session.topic.subject;
    const current = durationBySubject.get(subject.id) ?? {
      id: subject.id,
      name: subject.name,
      durationSeconds: 0,
    };
    current.durationSeconds += session.durationSeconds;
    durationBySubject.set(subject.id, current);
  }
  const daily = Array.from({ length: 7 }, (_, index) => {
    const date = new Date(today.getTime() - (6 - index) * 86_400_000)
      .toISOString()
      .slice(0, 10);
    return { date, durationSeconds: durationByDay.get(date) ?? 0 };
  });
  const streakAnchor = dayKeys.includes(todayKey)
    ? todayKey
    : new Date(today.getTime() - 86_400_000).toISOString().slice(0, 10);
  return {
    studiedDaysThisMonth: new Set(
      dayKeys.filter((key) => key.startsWith(month)),
    ).size,
    weeklySeconds: daily.reduce(
      (sum, item) => sum + item.durationSeconds,
      0,
    ),
    currentStreak: calculateStreak(dayKeys, streakAnchor),
    longestStreak: calculateLongestStreak(dayKeys),
    subjects: [...durationBySubject.values()]
      .filter((item) =>
        sessions.some(
          (session) =>
            startOfLocalDay(session.startedAt, timezone) >= weekStart &&
            session.topic.subject.id === item.id,
        ),
      )
      .map((item) => ({
        ...item,
