...
export const projectApi = {
  ...
  create: (input: { name: string; description: string }) =>
    apiRequest<ProjectDetailDto>("/api/projects", {
      method: "POST",
      body: JSON.stringify(input),
    }),
  update: (id: string, input: { name?: string; description?: string }) =>
    apiRequest<ProjectDetailDto>(`/api/projects/${id}`, {
      method: "PATCH",
      body: JSON.stringify(input),
    }),
  delete: (id: string) => apiRequest<void>(`/api/projects/${id}`, { method: "DELETE" }),
  createSubject: (projectId: string, name: string) =>
    apiRequest<SubjectDto>(`/api/projects/${projectId}/subjects`, {
      method: "POST",
      body: JSON.stringify({ name }),
    }),
  updateSubject: (subjectId: string, name: string) =>
    apiRequest<SubjectDto>(`/api/subjects/${subjectId}`, {
      method: "PATCH",
      body: JSON.stringify({ name }),
    }),
  ...
  createTopic: (subjectId: string, name: string) =>
    apiRequest<TopicDto>(`/api/subjects/${subjectId}/topics`, {
      method: "POST",
      body: JSON.stringify({ name }),
    }),
